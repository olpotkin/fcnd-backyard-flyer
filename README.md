...
[image_02]: ./docs/img-02.png "Axis"
[image_03]: ./docs/backyard_flyer_test.gif "Result"
[image_04]: ./docs/img-03.png "States of Flight"
...
Drone simulator is [available on GitHub](https://github.com/udacity/FCND-Simulator-Releases/releases) for all popular operating systems.
...
To communicate with the simulator (and a real drone) use the [UdaciDrone API](https://udacity.github.io/udacidrone/).
...
The `Drone` class contains the following attributes that can be useful for the project:

- `self.armed`: boolean for the drone's armed state
- `self.guided`: boolean for the drone's guided state (if the cript has control or not)
...
Each callback function shall be defined as a member function of the `BackyardFlyer` class provided in `backyard_flyer.py` that takes in only the `self` parameter.
...
The UdaciDrone API's `Drone` class also contains function to be able to send commands to the drone.  Here is a list of commands that may be useful during the project:
...
Note that when running the drone from `python drone.py` for manual flight, the telemetry log will default to "TLog-manual.txt".
